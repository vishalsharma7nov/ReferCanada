...
class JsonHolderCouponsListing {
    ...
    public JsonHolderCouponsListing(String json) {
        this.json = json;
    }

    protected List<SettingCouponsDetailsData> parseJSON(){
        JSONObject jsonObject = null;
        List<SettingCouponsDetailsData> list = new ArrayList<>();
        try {
           ...
           created_at               = new String[users.length()];
            ...
            for (int i = 0; i < users.length(); i++) {
                ...
                SettingCouponsDetailsData sd = new SettingCouponsDetailsData(jo.getString(KEY_ID),
